...
> ⚠️ **注意**: 本ドキュメントは 2025-10-27 時点の作業記録であり、現行 `master` の実装状態とは一致しない可能性があります。最新状況は `docs/DEV_HANDOFF_2025-12-12.md` 等を参照してください。
...
#### プロジェクト状態確認
- **コンパイル**: Unityエディタでエラーなしを確認
- **パッケージ**: Package Managerで依存関係問題なし
- **バージョン**: Unity 6000.2.2f1（現行）
- **Git**: 最新コミット反映済み、auto-merge-allowedラベルで自動マージ

#### 必須設定
- **Playerタグ**: PlayerControllerをアタッチしたオブジェクトに"Player"タグを付与（LOD参照のため）
- **シーン構成**: RuntimeTerrainManager + TileManager + PlayerControllerで基本動作確認

#### OpenSpecワークフロー遵守
1. 新機能は `/openspec:proposal [機能名]` で提案作成
2. 仕様洗練後 `/openspec:apply` で実装
3. 完了後 `/openspec:archive` でアーカイブ
4. 変更セットは `openspec/changes/` に格納

#### 次フェーズ準備（Phase 3: Deformシステム）
- Deformパッケージ（v1.2.2）のUnityPackageManagerからのインポート
- Mathematicsパッケージ（1.2.6）の確認
- Burstコンパイラ設定の有効化
- 変更セット: `openspec/changes/deform-system-investigation/`

#### 品質管理要件
- CI/CDテスト通過必須
- セキュリティスキャン: High/Critical脆弱性ゼロ
- コードカバレッジ: 80%以上目標
- 変更ログ: AI操作はJSON形式で記録

#### 緊急時の対応
- コンパイルエラー: 最新の変更をrevertし、安定版に戻す
- パフォーマンス低下: RealtimeUpdateSystemの設定調整
- OpenSpec不具合: `openspec list` で状態確認

#### 開発推奨事項
- Unityエディタで変更を即時確認
- 物理挙動変更時はRigidbody設定を注意
- 新規コンポーネントはVastcore名前空間を使用
- ドキュメントはdocs/に更新
