...
### ドキュメント訂正（ハイトマップ記述の見直し）
- 以前記載していた「ハイトマップ機能の追加」は、現時点では未実装／検証中です。
- 現状の地形生成は「ノイズベース」のみが動作対象です。
- ハイトマップのインポートやノイズとのハイブリッド生成は、将来の計画として残します（実装時に本ドキュメントを更新）。

### 使用方法（現状）
1. `RuntimeTerrainManager` もしくは `TileManager` をシーンに追加
2. ノイズパラメータ（`MeshGenerator.TerrainGenerationParams`）を調整
3. プレイヤーがいない場合は `Main Camera` が追跡対象として使用されます
4. 再生して周辺タイルが生成・更新されることを確認
...
（HeightMap / NoiseAndHeightMap は将来対応予定）
