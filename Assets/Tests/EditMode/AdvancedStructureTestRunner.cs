...
namespace Vastcore.Editor.Generation
{
    ...
    public class AdvancedStructureTestRunner : EditorWindow
    {
        ...
        {
            // 全ての記念碑タイプのテスト
            var monumentTypes = System.Enum.GetValues(typeof(AdvancedStructureTab.AdvancedShapeType));
            
            for (int i = 0; i < monumentTypes.Length; i++)
            {
                var type = (AdvancedStructureTab.AdvancedShapeType)monumentTypes.GetValue(i);
        ...
        private ProBuilderMesh CreateTestMonument(AdvancedStructureTab.AdvancedShapeType type)
        {
            // ここでの実装は、AdvancedStructureTabの生成ロジックの簡易版です
            // 実際のテストは、より統合された方法で行うべきです
