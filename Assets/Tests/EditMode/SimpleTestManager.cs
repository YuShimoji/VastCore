...
namespace Vastcore.Testing
{
    ...
    public class SimpleTestManager : MonoBehaviour
    {
        ...
        private Material CreateDefaultTerrainMaterial()
        {
            ...
            mat.name = "Advanced Terrain Material";
            
            // より自然な地形の色と質感
            mat.color = new Color(0.45f, 0.6f, 0.35f, 1f); // より自然な緑
            mat.SetFloat("_Metallic", 0.1f);               // 微かな金属感
            mat.SetFloat("_Smoothness", 0.3f);             // より自然な粗さ
            
            // テクスチャスケールを設定
            mat.SetTextureScale("_MainTex", new Vector2(50f, 50f));
            mat.SetTextureScale("_BumpMap", new Vector2(50f, 50f));
