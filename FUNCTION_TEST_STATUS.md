...
> ⚠️ **2025-12-05 更新**: CompositionTab.cs は UI スケルトンとして存在するが、CSG/Blend/Morph などコア処理ロジックは未実装。
> 以下のテーブルは「コア処理ロジック」の実装状況／テスト状況を表し、UI の有無は問わない。
> （従来の記載は過去の計画または別ブランチの内容を参照していたため、現状ベースに整理し直した。）

| 機能名 | 期待動作 | 実際の結果 | 状態 | 備考 |
|--------|----------|------------|------|------|
| **Union** | 2つのオブジェクトを結合 | ❓ 未確認 | 🔴 未実装（UIのみ） | CompositionTab.cs は存在するが CSG ロジック未実装 |
| **Intersection** | オブジェクトの交差部分を抽出 | ❓ 未確認 | 🔴 未実装（UIのみ） | 同上 |
| **Difference** | 最初のオブジェクトから2番目を減算 | ❓ 未確認 | 🔴 未実装（UIのみ） | 同上 |
...
| **Noise Blend** | ノイズによる表面変形 | ❓ 未確認 | 🔴 未実装（UIのみ） | 同上 |
| **Morph** | オブジェクト間のモーフィング | ❓ 未確認 | 🔴 未実装（UIのみ） | Advanced Operations UI のみ定義、処理未実装 |
| **Volumetric Blend** | 体積ベースブレンド | ❓ 未確認 | 🔴 未実装（UIのみ） | 同上 |
| **Distance Field** | 距離フィールド合成 | ❓ 未確認 | 🔴 未実装（UIのみ） | 同上 |

### 📈 成功率: 0/10 (0%) - コアロジック未実装。UI スケルトンのみ（CT-1 で段階的に実装予定）
...
| **Composition** | ⚪ UI スケルトンのみ | 🔴 未実装 | 🔴 未実装（設計段階） | UI は存在するが CSG/Blend 等のロジックは 0/10（CT-1 バックログ参照） |
...
> ℹ️ **注意**: このテスト結果は旧バージョン/別ブランチの実装を対象としており、現行の StructureGenerator および Random/Composition タブの実装状態とは一致しません。現状のコード状態の評価には本ドキュメント上部の各セクションと `docs/ISSUES_BACKLOG.md` を参照してください。
