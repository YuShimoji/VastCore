...
- 現フェーズ: Phase D 進行中。SP-019 建物定義仕様 Phase 1-3 実装済み
- 直近の状態 (2026-03-18 session 4):
  - SP-019 建物定義仕様 (タグ重み複合体方式) Phase 1-3 実装完了 (partial/65%)
    - Phase 1: StructureTagProfile + StructureTagPreset SO + BlendScore + テスト20件
    - Phase 2: StructureTagAdapter (38enum→タグ変換) + StampExporter自動タグ付与 + テスト15件
    - Phase 3: ComponentVariant + ComponentSelector + BuiltInComponentVariants (22種) + テスト13件
  - PrefabStampDefinition に tagProfile フィールド追加
  - DECISION LOG 5件追加 (タグ方式/構成要素/接続設計/配置/スタイル)
  - spec-index 33エントリ (SP-019追加)
  - 次: Unity実機検証 (コンパイル+テスト48件) → Phase 4 配置ルール / Phase 5 マテリアル
