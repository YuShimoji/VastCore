...
- 直近の状態 (2026-03-17 夜間自走):
  - local/origin分岐統合完了 (22+16 commits, 12競合解決)
  - Phase C完了 + SG-1/SG-2 + PD-4 + パイプライン統合 全マージ済み
  - spec-index.json整合: SP-009(→partial/75%), SP-016 Erosion新規追加, SP-010重複解消(→SP-015)
  - DOCS_INDEX.md 10件の未登録エントリ追加
  - コード内TODO残件: 1件のみ (Billboard未実装, 低優先)
  - 未決定(HUMAN_AUTHORITY): 最終体験像、「ユニーク」の実現手段、StructureGenerator→Stamp統合方針
  - SP-017 StampExporter実装完了: StructureGenerator→Prefab→StampDefinition変換ブリッジ
  - 次: Unity実機検証 → "1構造物が地形に立つ"体験スライス確認 → Phase D着手判断
